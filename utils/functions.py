...
def load_model(opt):
    dir = 'Checkpoints/%s/%s' % (opt.model_name, opt.model_param)
...
def generate_dir2save(opt):
    ...
    if opt.mode == 'train':
        ...
        dir2save = 'Checkpoints/%s/scale_factor=%.3f,min_size=%d,niter=%d,lambda_cyc=%.3f,lambda_idt=%.3f' % (opt.input_name,opt.scale_factor_init,opt.min_size,opt.niter,opt.lambda_cyc,opt.lambda_idt)
    if opt.mode == 'transfer':
        dir2save = 'Transfer/%s/%s/%s' % (opt.model_name, opt.model_param, opt.input_name[:-4])
    return dir2save
...
def my_plot(D_loss,G_loss,R_loss,C_loss,opt):
    fig,ax = plt.subplots(1)
    D_loss_line = plt.plot(D_loss, label='D_loss')
    G_loss_line = plt.plot(G_loss, label='G_loss')
    R_loss_line = plt.plot(R_loss, label='R_loss')
    C_loss_line = plt.plot(C_loss, label='C_loss')
    plt.title('loss')
    plt.ylabel('loss')
    plt.xlabel('epochs')
    plt.ylim(-2.0,3.0)
    plt.legend(loc=1)
    plt.savefig('%s/plot.png' % opt.out_)
    plt.close()    

def my_plot2(D_loss2,G_loss2,R_loss2,C_loss2,opt):
    fig,ax = plt.subplots(1)
    D_loss_line = plt.plot(D_loss2, label='D2_loss')
    G_loss_line = plt.plot(G_loss2, label='G2_loss')
    R_loss_line = plt.plot(R_loss2, label='R2_loss')
    C_loss_line = plt.plot(C_loss2, label='C2_loss')
    plt.title('loss')
    plt.ylabel('loss')
    plt.xlabel('epochs')
    plt.ylim(-2.0,3.0)
    plt.legend(loc=1)
    plt.savefig('%s/plot2.png' % opt.out_)
    plt.close()    
