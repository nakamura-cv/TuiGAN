...
def TuiGAN_transfer(Gs,Zs,reals,NoiseAmp,Gs2,opt,in_s=None,gen_start_scale=0):
    if in_s is None:
        in_s = torch.full(reals[0].shape, 0, device=opt.device)
    x_ab = in_s
    x_aba = in_s
    ...
    for G,G2,Z_opt,real_curr,real_next,noise_amp in zip(Gs,Gs2,Zs,reals,reals[1:],NoiseAmp):
        ...
        count += 1
        plt.imsave('%s/x_ab_%d.png' % (dir2save,count), functions.convert_image_np(x_ab.detach()), vmin=0,vmax=1)        
        plt.imsave('%s.png' % (dir2save), functions.convert_image_np(x_ab.detach()), vmin=0,vmax=1)
